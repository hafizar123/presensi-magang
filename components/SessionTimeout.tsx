...
export default function SessionTimeout() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (lastStamp) {
        ...
        if (timePassed > TIMEOUT_MS && !isPublicPage) {
          setIsOpen(true);
        }
      }
    }, 1000);
  ...
  }, [status, updateActivity, isOpen, TIMEOUT_MS, isPublicPage]);

  if (status !== "authenticated" || isPublicPage) return null;
      ...
      <AlertDialogContent className="!w-[78vw] sm:!w-full sm:!max-w-[400px] p-0 border-none bg-transparent shadow-none outline-none overflow-hidden">
        <div className="relative bg-white/95 dark:bg-[#0c0a09]/95 border border-white/20 dark:border-[#292524] rounded-[2.5rem] p-8 sm:p-12 shadow-2xl backdrop-blur-xl flex flex-col items-center overflow-hidden">
          ...
          <AlertDialogHeader className="flex flex-col items-center justify-center space-y-5 w-full">
            <div className="h-14 w-14 sm:h-16 sm:w-16 bg-slate-50 dark:bg-[#1c1917] rounded-2xl flex items-center justify-center border border-slate-100 shadow-sm">
              <Timer className="h-8 w-8 text-[#99775C]" />
            </div>
            <div className="space-y-2 text-center w-full">
              ...
              <AlertDialogDescription className="text-slate-500 dark:text-gray-400 text-[10px] sm:text-sm leading-relaxed w-full">
                Masa berlaku sesi Anda telah berakhir demi keamanan data. Silakan melakukan autentikasi ulang untuk melanjutkan aktivitas.
              </AlertDialogDescription>
            </div>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8 w-full flex justify-center">
            <AlertDialogAction onClick={handleLogout} className="w-full sm:flex-1 h-9 sm:h-11 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-[10px] sm:text-xs m-0 order-1 sm:order-2 shadow-lg shadow-red-600/10 border-none">
              Kembali ke Beranda
            </AlertDialogAction>
          </AlertDialogFooter>
        </div>
