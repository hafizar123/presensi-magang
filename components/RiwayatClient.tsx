  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#EAE7DD] dark:bg-[#0c0a09] border-r border-[#d6d3c9] dark:border-[#1c1917] transition-colors duration-300">
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2">
            ...
            <Link href="/dashboard" className="flex items-center gap-3 px-4 py-3 text-[#5c4a3d] dark:text-[#EAE7DD] hover:bg-white/50 dark:hover:bg-[#1c1917]/50 hover:text-[#99775C] dark:hover:text-white rounded-xl font-medium transition-all group">
      ...
      <main className={`pt-24 px-4 md:px-8 pb-12 transition-all duration-300 ease-in-out ${isSidebarOpen ? "md:ml-[280px]" : "md:ml-0"}`}>
        ...
        <Tabs defaultValue="in" className="w-full space-y-6">
            <div className={`transition-all duration-1000 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${startAnimation ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
                ...
                <TabsList className="grid w-full grid-cols-2 bg-slate-100 dark:bg-[#1c1917] p-1 rounded-xl h-auto border dark:border-[#292524] mb-4">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {filteredLogs.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={4} className="h-32 text-center text-slate-400">Data tidak ditemukan.</TableCell>
                                        </TableRow>
                                    ) : (
                                        filteredLogs.map((log, i) => (
                                            <TableRow 
                                                key={log.id} 
                                                className="border-b border-slate-100 dark:border-[#292524] hover:bg-slate-50 dark:hover:bg-[#292524]
                                            ...
                                            >
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center gap-3">
                                                        <div className="p-2 bg-slate-100 dark:bg-[#292524] rounded-lg"><CalendarDays className="h-4 w-4 text-slate-500" /></div>
                                                        <div>
                                                ...
                                                <TableCell><div className="font-mono font-medium text-slate-700 dark:text-slate-300">{log.time} WIB</div></TableCell>
                                                <TableCell className="text-right pr-6">
                                                    {log.status === "HADIR" && <Badge className="bg-green-100 text-green-700 border-none">Tepat Waktu</Badge>}
                                                    {log.status === "TELAT" && <Badge className="bg-orange-100 text-orange-700 border-none">Terlambat</Badge>}
                                                    {log.status === "IZIN" && <Badge className="bg-blue-100 text-blue-700 border-none">Izin</Badge>}
                                                </TableCell>
                                            </TableRow>
                                        ))
                                    )}
                    ...
                    <Card className="border-none shadow-md bg-white dark:bg-[#1c1917] overflow-hidden">
                        ...
                        <CardContent className="p-0">
                            <Table>
                                ...
                                <TableBody>
                                    {filteredLogs.map((log, i) => (
                                        <TableRow key={log.id} className="border-b border-slate-100 dark:border-[#292524] hover:bg-slate-50 dark:hover:bg-[#292524] animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${i * 50}ms` }}>
                                            <TableCell className="text-center font-medium text-slate-500 pl-4">{i + 1}</TableCell>
                                            <TableCell>
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-slate-100 dark:bg-[#292524] rounded-lg"><CalendarDays className="h-4 w-4 text-slate-500" /></div>
