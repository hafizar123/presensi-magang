...
export default function ProfileClient({ user }: ProfileClientProps) {
  ...
  // State buat nampung Settingan Admin (Jam Operasional)
  ...
  // --- LOGIC JAM OPERASIONAL (DINAMIS DARI DB) ---
  const getOperationalInfo = () => {
    // Kalau data belum ke-load, kasih placeholder
    ...
    // Fallback value kalau settingan admin kosong (Default Value)
  ...
  // ... (SISA KODE HANDLER IMAGE, UPDATE PROFILE, DLL SAMA SEPERTI SEBELUMNYA) ...
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            <div className="flex justify-center mb-8">
                <TabsList className="bg-slate-200/60 dark:bg-[#1c1917] p-1.5 rounded-full h-14 w-full max-w-sm grid grid-cols-2 gap-2 shadow-inner">
                    <TabsTrigger value="overview" className="rounded-full h-full text-slate-500 font-semibold data-[state=active]:bg-[#99775C] dark:data-[state=active]:bg-[#3f2e26] data-[state=active]:text-white transition-all">Ringkasan</TabsTrigger>
                    <TabsTrigger value="settings" className="rounded-full h-full text-slate-500 font-semibold data-[state=active]:bg-[#99775C] dark:data-[state=active]:bg-[#3f2e26] data-[state=active]:text-white transition-all">Edit & Keamanan</TabsTrigger>
                </TabsList>
            </div>
            ...
            <TabsContent value="overview" className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    {/* JAM OPERASIONAL DINAMIS (UPDATED HERE) */}
