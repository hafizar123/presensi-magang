...
import { 
  ...
  LogOut, Bell, ClipboardList, Menu 
} from "lucide-react";
...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  // --- KONTEN SIDEBAR (Sama persis styling User) ---
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-[#EAE7DD] dark:bg-[#0c0a09] border-r border-[#d6d3c9] dark:border-[#1c1917] transition-colors duration-300">
        ...
        <div className="h-16 flex items-center gap-3 px-6 bg-[#99775C] dark:bg-[#271c19] text-white border-b border-[#8a6b52] dark:border-[#3f2e26]">
             ...
             <span className="font-bold text-lg tracking-tight">SIP-ADMIN</span>
        </div>

        {/* MENU LIST: Body Krem / Warm Black */}
              ...
              return (
                <Link 
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all group ${
                    isActive 
                      ? "bg-[#99775C] dark:bg-[#3f2e26] text-white shadow-md font-bold" 
                      : "text-[#5c4a3d] dark:text-[#EAE7DD] hover:bg-white/50 dark:hover:bg-[#1c1917]/50 hover:text-[#99775C] dark:hover:text-white"
                  }`}
                >
                  <item.icon className={`h-5 w-5 ${isActive ? "text-white" : "group-hover:text-[#99775C] dark:group-hover:text-white"}`} />
                  {item.label}
                </Link>
              );
            })}

            {/* LOGOUT: Langsung di bawah menu terakhir (Sesuai Request) */}
            <div className="pt-2">
                <LogoutModal>
                    <button className="w-full flex items-center gap-3 px-4 py-3 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-xl font-medium transition-all text-left group">
                        <LogOut className="h-5 w-5 group-hover:scale-110 transition-transform" /> Keluar Admin
                    </button>
                </LogoutModal>
            </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F2F5F8] dark:bg-[#0c0a09] font-sans transition-colors duration-300">
      ...
      {/* NAVBAR: Coklat Sorrell / Espresso */}
      <nav 
          ...
          <div className="flex items-center gap-3 text-white">
            ...
            <div className="flex items-center gap-3 pl-1">
                <div className="hidden md:flex flex-col items-end">
                    ...
                    <span className="text-[10px] text-[#EAE7DD]/80 font-medium">Disdikpora DIY</span>
                </div>
                <Avatar className="h-9 w-9 border-2 border-white/20">
                    <AvatarImage src="https://github.com/shadcn.png" />
                    <AvatarFallback className="bg-[#5c4a3d] text-white">AD</AvatarFallback>
                </Avatar>
            </div>
          </div>
      </nav>
