...
export default function EditProfilePage() {
  ...
  // --- FITUR UPLOAD FOTO (YANG KEMAREN ILANG) ---
    ...
    try {
      const res = await fetch("/api/profile", {
        ...
        body: JSON.stringify({
          ...
          image: formData.image // Kirim URL foto ke backend buat disave permanen
        })
      });
    ...
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-black font-sans pb-20 transition-colors duration-300">
      ...
      <header className="fixed top-0 left-0 right-0 h-14 md:h-16 bg-white/70 dark:bg-black/70 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/50 z-50 flex items-center justify-between px-4 md:px-6 transition-all">
        <div className="flex items-center gap-2">
            <Button 
                ...
                size="icon" 
                onClick={() => router.back()}
                className="text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 -ml-2 rounded-full transition-colors"
            >
                <ChevronLeft className="h-6 w-6" />
            </Button>
            <span className="md:hidden text-blue-600 dark:text-blue-400 font-medium text-base cursor-pointer" onClick={() => router.back()}>
                Kembali
            </span>
        </div>
        
        <h1 className="text-base md:text-lg font-semibold text-slate-900 dark:text-white absolute left-1/2 -translate-x-1/2">
            Edit Profil
        </h1>
      ...
      <main className="pt-24 md:pt-28 px-4 md:px-0 max-w-xl mx-auto space-y-8">
        ...
        <div className="flex flex-col items-center">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                ...
                <Avatar className="h-32 w-32 md:h-40 md:w-40 border-[6px] border-white dark:border-zinc-900 shadow-2xl relative z-10 transition-transform duration-300 group-hover:scale-105">
                    {/* Logic Gambar: Pake foto upload user, kalo ga ada pake inisial */}
                    <AvatarImage 
                        src={formData.image || `https://ui-avatars.com/api/?name=${formData.name || 'User'}&background=007AFF&color=fff&size=256`} 
        ...
        <div className="space-y-6">
            ...
            {/* GROUP 1: Identitas - KOTAK (rounded-md) */}
                    ...
                    <div className="flex-1 ml-4 border-b border-slate-100 dark:border-slate-800 py-3 flex items-center justify-between">
                         <div className="flex flex-col w-full">
                            ...
                            <Input 
                                ...
                                className="border-none shadow-none p-0 h-auto font-medium text-base md:text-lg bg-transparent focus-visible:ring-0 placeholder:text-slate-300 dark:placeholder:text-slate-600 text-slate-900 dark:text-white"
                    ...
                    <div className="flex-1 ml-4 py-3 flex items-center justify-between">
                         <div className="flex flex-col w-full">
                            ...
                            <Input 
                                ...
                                className="border-none shadow-none p-0 h-auto font-medium text-base text-slate-900 dark:text-white bg-transparent focus-visible:ring-0"
            ...
            {/* GROUP 2: Keamanan - KOTAK (rounded-md) */}
                    ...
                    <div className="flex items-center pl-6 pr-6 py-2 relative">
                        ...
                        <div className="flex-1 ml-4 border-b border-slate-100 dark:border-slate-800 py-3 flex items-center">
                            <Input 
                                ...
                                className="border-none shadow-none p-0 h-auto font-medium text-base bg-transparent focus-visible:ring-0 placeholder:text-slate-300 dark:placeholder:text-slate-600 text-slate-900 dark:text-white w-full"
        ...
        <div className="pt-4 md:pt-8 pb-8">
            {isSuccess ? (
                ...
                <Button 
                    onClick={handleSave} 
                    disabled={isSaving}
                    className="w-full h-14 rounded-xl bg-[#007AFF] hover:bg-blue-600 active:scale-95 text-white shadow-xl shadow-blue-500/20 text-lg font-bold transition-all disabled:opacity-70 disabled:scale-100"
                >
