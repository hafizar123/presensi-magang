...
import { 
  ...
  CheckCircle2, Loader2, Filter, Eye, Download
} from "lucide-react";
...
// Update Tipe Data Sesuai Database
type LeaveRequest = {
  id: string;
  user: { name: string; email: string };
  date: string;       // <--- INI YG BENER (Bukan range)
  reason: string;
  proofFile: string | null; // <--- INI FILE BUKTI
  ...
  return (
    <div className="space-y-6 pb-10">
        ...
        <AlertDialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl">
            <AlertDialogHeader>
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${actionType === 'REJECTED' ? 'bg-red-100 dark:bg-red-900/20 text-red-600' : 'bg-blue-100 dark:bg-blue-900/20 text-blue-600'}`}>
            ...
            <AlertDialogFooter className="mt-2">
                <AlertDialogCancel className="bg-slate-100 dark:bg-slate-800 border-0 rounded-lg">Batal</AlertDialogCancel>
                <AlertDialogAction 
      ...
      <Dialog open={isSuccessOpen} onOpenChange={setIsSuccessOpen}>
        <DialogContent className="sm:max-w-[400px] bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 p-0 overflow-hidden rounded-2xl">
            <div className="flex flex-col items-center justify-center py-10 px-6 text-center animate-in zoom-in-95 duration-300">
                ...
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Berhasil Diproses!</h2>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Approval Izin</h1>
          <p className="text-slate-500 dark:text-slate-400">Validasi pengajuan izin sakit atau cuti magang.</p>
        </div>
      ...
      <div className="grid gap-4">
        {loading ? (
            ...
            filteredRequests.map((req) => (
                <Card key={req.id} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition-all">
                    <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                            <div className="flex items-start gap-3">
                                <div className="p-2 bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-full mt-1">
                    ...
                    <CardContent>
                        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-100 dark:border-slate-800 space-y-3">
                            {/* ALASAN */}
                            <div className="flex items-start gap-3">
                                <span className="text-slate-400 text-xs w-20 flex-shrink-0 font-medium">Keterangan</span>
                            ...
                            {/* TANGGAL (FIXED) */}
                            <div className="flex items-center gap-3">
                                <span className="text-slate-400 text-xs w-20 flex-shrink-0 font-medium">Tanggal Izin</span>
                                <div className="flex items-center gap-2 text-sm text-slate-900 dark:text-slate-200 font-bold bg-white dark:bg-slate-900 px-2 py-1 rounded border border-slate-200 dark:border-slate-700">
                            ...
                            {/* FILE BUKTI (NEW) */}
                            <div className="flex items-center gap-3">
                                <span className="text-slate-400 text-xs w-20 flex-shrink-0 font-medium">Lampiran</span>
                                {req.proofFile ? (
                                    <a 
                                        href={`/uploads/${req.proofFile}`} // Asumsi file di folder public/uploads
                                        target="_blank" 
                                        rel="noopener noreferrer"
                                    >
                                        <Button variant="outline" size="sm" className="h-8 text-xs border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 dark:bg-blue-900/30 dark:border-blue-800 dark:text-blue-300">
                                            <Eye className="h-3 w-3 mr-2" />
                                            Lihat Surat
                                        </Button>
                                    </a>
                                ) : (
                                    <span className="text-xs text-slate-400 italic">Tidak ada lampiran file.</span>
                                )}
                            </div>
                        </div>
                        ...
                        {req.status === "PENDING" && (
                            <div className="flex gap-3 mt-4 justify-end">
                                <Button 
                                    variant="outline" 
                                    className="text-red-600 border-red-200 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-700 h-9"
                                ...
                                <Button 
                                    className="bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-600/20 h-9"
